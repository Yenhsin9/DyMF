...
def initialize_adjacency_matrix(batch_size, encode_length, shot_type):
    ...
    # adjacency_matrix.shape = (14, encode_length * 2, encode_length * 2)  tile adjacency_matrix, (batch_size, 1, 1, 1)
    # To(batch_size, 14, encode_length * 2, encode_length * 2)
...
class relational_GCN_layer(nn.Module):
    def __init__(self, hidden_size, type_num, num_basis, device):
        ...
        self.linear_combination = torch.nn.Parameter(torch.Tensor(type_num - 1 + 3, num_basis))
        self.dropout = nn.Dropout(0.1)
    ...
    def forward(self, node_embedding, adjacency_matrix, activation_function):
        mutil_relational_weight = torch.matmul(self.linear_combination, self.basis_matrix.view(self.num_basis, -1)).view(self.type_num - 1 + 3, self.hidden_size, self.hidden_size)
...
class Decoder(nn.Module):
    def __init__(self, args, device):
        ...
        self.rGCN = relational_GCN(hidden_size, type_num, 3, num_layer, device)
        ...
        if train:
            for batch in range(batch_size):
                if step % 2 == 0:
                    ...
                    adjacency_matrix[batch][13][(step - 2) * 2][(step - 1) * 2 + 1] = 1
                    adjacency_matrix[batch][13][(step - 1) * 2 + 1][(step - 2) * 2] = 1
                if step % 2 == 1:
                    ...
                    adjacency_matrix[batch][13][(step - 1) * 2][(step - 2) * 2 + 1] = 1
                    adjacency_matrix[batch][13][(step - 2) * 2 + 1][(step - 1) * 2] = 1
...
class Encoder(nn.Module):
    def __init__(self, args, device):
        ...
        self.rGCN = relational_GCN(hidden_size, type_num, 3, num_layer, device) # into 2 type (passive and active) and padding
        self.gcn = GCN(args['hidden_size'], args['hidden_size'], 0.1, num_layer, args, device)
    ...
    def forward(self, player, shot_type, player_A_x, player_A_y, player_B_x, player_B_y, encode_length):
        ...
        coordination_sequence = torch.stack((player_A_coordination, player_B_coordination), dim=2).view(player.size(0), -1, 2)
        coordination_transform = self.coordination_transform(coordination_sequence)
        coordination_transform = F.relu(coordination_transform) #Relu(W*L)

        player = player.repeat([1, encode_length])
        player_embedding = self.player_embedding(player)

        rally_information = torch.cat((coordination_transform, player_embedding), dim=-1) #concat with player
        
        model_input = self.model_input_linear(rally_information) #The initial resentation of the node E
        ...
        player_A_embedding = model_input[:, 0::2, :].clone()
        player_B_embedding = model_input[:, 1::2, :].clone()
        # (batch_size, encode_length, encode_length)
        ...
        # node_embedding.shape = (batch_size, encode_length * 2, hidden_size)
        ...
        #Let Player A and Player B influence each other
        player_A_node_embedding = player_A_node_embedding + B_weight.unsqueeze(1) * player_B_node_embedding
        player_B_node_embedding = player_B_node_embedding + A_weight.unsqueeze(1) * player_A_node_embedding
